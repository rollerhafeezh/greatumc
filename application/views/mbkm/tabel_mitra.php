							...
							<table id="datatabel" class="table table-striped" style="width:100%">
								<thead>
		                            <tr>
		                                ...
		                                <th>Alamat</th>
		                                <!-- <th>Periode</th> -->
		                                <th width="1" class="text-nowrap">Kontak Mitra</th>
		                                <th width="1">Aktif</th>
		                                <th width="1">Aksi</th>
		                            </tr>
		                        </thead>
	  ...
	  <div class="modal-body">
	  	...
	  	<div class="row">
			<div class="col-md-6 mb-2">
				...
				<select id="kode_fak" name="kode_fak" class="form-select"  onchange="ref_prodi()" disabled>
			...
			<div class="col-md-6 mb-2">
	  		...
	  		<div class="col-md-6 mb-2">
				...
				<select id="id_smt" name="id_smt" class="form-select" disabled="">
...
function hapus(id) {
	var konfirmasi = confirm('Hapus Mitra Yang Dipilih ?')
	if (konfirmasi) {
		var formData = new FormData()
		formData.append('id_mitra', id)

		fetch('<?= base_url('mbkm/hapus/mitra') ?>', { method: 'POST', body: formData })
		.then(response => response.text())
		.then(text => {
			table.ajax.reload(null,false);
			Toastify({
				text: "Info: Mitra kampus merdeka berhasil dihapus.",
			}).showToast();
		})
	}
...
function aktif(id, aktif) {
	var konfirmasi = confirm('Ganti Status Aktif Mitra ?')
	if (konfirmasi) {
		var formData = new FormData()
		formData.append('id_mitra', id)
		formData.append('aktif', aktif)

		fetch('<?= base_url('mbkm/aktif/mitra') ?>', { method: 'POST', body: formData })
		.then(response => response.text())
		.then(text => {
			table.ajax.reload(null,false);
			Toastify({
...
$(document).ready(function() {
	...
	table = $('#datatabel').DataTable( {
		...
		buttons: [
			...
			<?php if ($_SESSION['app_level'] == 7): ?>
			{
                ...
                action: function ( e, dt, node, config ) {
                    window.location.href = '<?= base_url ('mbkm/tambah_mitra') ?>'
                }
            },
        	<?php endif; ?>
		],
	...
	$('#kode_fak, #kode_prodi, #id_smt, #id_jenis_aktivitas_mahasiswa, #status, #jenis_program, #aktif').change(function(){ //button filter event click
        table.ajax.reload(null,false);  //just reload table
	});
