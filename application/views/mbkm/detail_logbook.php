...
<div class="row">
	<div class="col-md-12">
		<div class="form-group mb-3">
			...
			<select name="status" id="status" required="" class="form-select" data-id="<?= sha1($detail->id_logbook) ?>" onchange="ubah_status(this, `logbook`)">
	...
	<div class="col-md-12">
		<div class="form-group">
			<label class="form-label text-dark" >Tgl. Kegiatan</label>
			<p><?= tanggal_indo($detail->tgl_kegiatan) ?></p>
		</div>
	</div>
	<div class="col-md-12">
		<div class="form-group">
	...
	<div class="col-md-12 mb-3 <?= $detail->file == '' ? 'd-none' : '' ?>">
		<div class="form-group">
			...
			<a class="d-block" href="<?= $detail->file ?>" download><i class="fa fa-download me-1"></i> Unduh Lampiran</a>
		</div>
	</div>
	<div class="col-md-12 mb-3 <?= $detail->file_gambar == '' ? 'd-none' : '' ?>">
		<div class="form-group">
			<label class="form-label text-dark" >Lampiran Foto Kegiatan</label>
			<a class="d-block" href="<?= $detail->file_gambar ?>" download title=""><i class="fa fa-download me-1"></i> Unduh Lampiran</a>
		</div>
	</div>
</div>
...
<div class="tanggapan row" style="display: <?= ($detail->status != 0 OR $detail->status == '') ? 'none' : ''; ?>;">
	<div class="col-md-12 mb-3">
		...
		<?php if ($_SESSION['app_level'] == 2): ?>
		<form onsubmit="event.preventDefault(); simpan_tanggapan(this, `<?= sha1($detail->id_logbook) ?>`, `logbook`)" class="mb-3">
			<input type="hidden" name="id_parent" value="<?= $detail->id_logbook ?>">
					      	...
					      	<div class="timeline">
					      		...
					      		<div class="tl-entry">
					                ...
					                <div class="tl-content card ">
					                    <div class="card-body">
					                    	<?php if ($_SESSION['app_level'] == 2): ?>
					                    	<a class="badge badge-super bg-danger text-decoration-none text-white" href="javascript:void(0)" onclick="hapus_tanggapan(`<?= sha1($row->id_tanggapan) ?>`, `<?= sha1($row->id_parent) ?>`, `logbook`)">
