	...
	<div class="content__wrap">
		<div class="row">
			...
			<div class="col-12 mb-4">
				<div class="card h-100">
					<div class="card-body">
					<h5 class="card-title">Jadwal <?=strtoupper($jenis)?></h5>
					<form method="post" action="<?= base_url('kelas/proses_ganti_jadwal_ujian/'.$jenis.'/'.$id_kelas_kuliah) ?>">
						<input type="hidden" id="id_smt" value="<?=$kelas->id_smt?>" name="id_smt">
						<div class="row g-3">
							<div class="col-md-4">
								<label for="tgl" class="form-label">Tanggal Ujian</label>
								<input id="tgl" name="tgl" onfocus="this.select()" type="date" class="form-control" onchange="sembunyikan_gedung()">
							</div>
							<div class="col-md-4">
...
function ref_ruangan_kuliah()
{
	...
	fetch('<?=base_url('kelas/ref_ruangan_ujian/')?>', {
		method: 'POST',
		headers: {
