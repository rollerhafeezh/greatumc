...
<div id="pembimbing" class="tab-pane fade" role="tabpanel" aria-labelledby="pembimbing-tab">
	<form class="mb-3" method="POST" action="<?= base_url('studi_akhir/tambah_dosen/pembimbing') ?>">
		...
		<div class="row">
			...
			<div class="col-md-5 mb-1">
				<select name="id_kategori_kegiatan" class="form-select" required="">
					...
					<?php $kategori_kegiatan = $this->Aktivitas_model->kategori_kegiatan($id_kategori_kegiatan)->result(); ?>
		...
		<table class="table table-bordered table-striped my-0">
			...
			<?php if (count($pembimbing) > 0): ?>
				...
				<tr>
					...
					<td>
						<a onclick="return confirm('Hapus Dosen Pembimbing a.n <?= $pembimbing->nm_sdm ?> ?')" href="<?= base_url('studi_akhir/hapus_dosen/pembimbing/'.$pembimbing->id_pembimbing.'/'.$pembimbing->id_aktivitas) ?>" class="badge bg-danger text-white">hapus</a>
					</td>
				</tr>
				<?php endforeach; ?>
			<?php else: ?>
