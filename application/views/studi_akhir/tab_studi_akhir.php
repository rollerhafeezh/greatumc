<div id="studi_akhir" class="tab-pane fade show active" role="tabpanel" aria-labelledby="studi_akhir-tab">
	...
	<form class="form-aktivitas-mahasiswa was-validated" action="<?= base_url('studi_akhir/update') ?>" method="POST" enctype="multipart/form-data">
		<input type="hidden" name="id_aktivitas" value="<?= $aktivitas->id_aktivitas ?>">
		<div class="form-group row mb-2">
			<label class="col-md-3 col-form-label" for="status">Status Kegiatan <span class="text-danger">*</span></label>
			<div class="col-md-3">
				<select id="status" name="status" class="form-select" required="" autofocus="">
					<option value="" hidden>Pilih Status</option>			
					<option value="0" <?= ($aktivitas->status == 0) ? 'selected' : '' ?> >Belum Selesai</option>}
					<option value="1" <?= ($aktivitas->status == 1) ? 'selected' : '' ?> >Sudah Selesai</option>}
					option
				</select>
			</div>
		</div>
			...
			<div class="col-md-3">
				<select id="jenis_aktivitas" name="id_jenis_aktivitas_mahasiswa" class="form-select id_kat_mk" required="" <?= $_SESSION['kode_fak'] == '0' ? 'disabled' : 'disabled' ?>>
					...
					<?php foreach($jenis_aktivitas_mahasiswa as $row): ?>
						<?php if ($row->id_jenis_aktivitas_mahasiswa == 2 || $row->id_jenis_aktivitas_mahasiswa == 6): ?>
		...
		<div class="form-group row mb-2">
			...
			<div class="col-md-6">
				<textarea class="form-control" rows="5" id="judul" name="judul" placeholder="Judul Masih Kosong" ><?= $aktivitas->judul ?></textarea>
				<div class="invalid-feedback">Silahkan konfirmasi kepada mahasiswa untuk memasukan judul.</div>
			</div>
		</div>
		<div class="form-group row mb-2">
			<label class="col-md-3 col-form-label" for="judul_en">Judul (English) <span class="text-danger">*</span></label>
			<div class="col-md-6">
				<textarea class="form-control" rows="5" id="judul_en" name="judul_en" placeholder="Judul (English) Masih Kosong" ><?= $aktivitas->judul_en ?></textarea>
				<div class="invalid-feedback">Silahkan konfirmasi kepada mahasiswa untuk memasukan judul dalam bahasa inggris.</div>
			</div>
		</div>
		<div class="form-group row mb-2">
			...
			<div class="col-md-6">
				...
				<div class="invalid-feedback">Unggah file surat penugasan pembimbing. Maksimal 10 MB.</div>
			 	<?php if ($aktivitas->file_sk_tugas != ''): ?>
			        ...
			        <a class="text-decoration-none float-end cursor-pointer" href="javascript:void(0)" onclick="lihat_berkas(`File SK Tugas`, `<?= $aktivitas->file_sk_tugas ?>?time=<?= time() ?>`)"><img src="<?= base_url('assets/img/pdf.png') ?>" style="margin-top: -4px;"> Lihat Berkas SK Tugas (pdf)</a>
		    	<?php endif; ?>
			</div>
		</div>
		...
		<div class="form-group row mb-2">
			...
			<div class="col-md-6">
				<textarea class="form-control" rows="5" id="keterangan" name="keterangan" placeholder="Ringkasan <?= $nama_kegiatan ?> (Opsional)"><?= $aktivitas->keterangan ?></textarea>
			</div>
		</div>
