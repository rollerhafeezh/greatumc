<?php $cek_penjadwalan = $this->Aktivitas_model->penjadwalan('p.id_aktivitas = "'. $aktivitas->id_aktivitas.'" AND "'.date('Y-m-d_H:i:s').'" >= p.tanggal AND p.status = "0" AND p.id_kegiatan = '.$this->uri->segment(4))->row(); ?>
            ...
            <div class="col-md-12">
                ...
                <script>
                    ...
                    const options = {
                        ...
                        userInfo: {
                            displayName: '<?= format_nama($_SESSION['nama_pengguna']) ?> (KETUA SIDANG)'
                        }
                    };
                    ...
                    api.on('readyToClose', () => {
                        if(confirm('Akhiri dan tutup video conference ? \n*) video conference yang sudah diakhiri tidak bisa dibuka kembali.')) {
                            window.location.href = '<?= base_url('bimbingan/tutup_seminar_sidang/'.$cek_penjadwalan->id_penjadwalan.'/'.$cek_penjadwalan->id_aktivitas) ?>'
                        }
                    });
                </script>
            </div>
            <div class="col-md-12">
                <button class="btn btn-danger w-100" onclick="if(confirm('Akhiri dan tutup <?= format_nama($cek_penjadwalan->deskripsi) ?> ? \n*) <?= format_nama($cek_penjadwalan->deskripsi) ?> yang sudah diakhiri tidak bisa dibuka kembali.')) {
                            window.location.href = '<?= base_url('bimbingan/tutup_seminar_sidang/'.$cek_penjadwalan->id_penjadwalan.'/'.$cek_penjadwalan->id_aktivitas) ?>'
                        }">Akhiri <?= format_nama($cek_penjadwalan->deskripsi) ?></button>
            </div>
        </div>
    </div>
</div>
