...
class Rekapitulasi_data_model extends CI_Model {
    ...
    {
        $this->db->select('d.nm_sdm,f.inisial_fak,p.nama_prodi,c.nm_mk,b.nm_kls,e.sks_subst_tot,GROUP_CONCAT(DISTINCT(a.tanggal)) as kehadiran, COUNT(DISTINCT(a.tanggal)) as jml');
        //$this->db->select('d.nm_sdm,f.inisial_fak,p.nama_prodi,c.nm_mk,b.nm_kls,2 as sks_subst_tot,GROUP_CONCAT(a.tanggal) as kehadiran, COUNT(a.tanggal) as jml');
        ...
        $this->db->join($_ENV['DB_GREAT'].'ajar_dosen e','e.id_kelas_kuliah=a.id_kelas_kuliah');
		$this->db->join($_ENV['DB_GREAT'].'kelas_kuliah b','e.id_kelas_kuliah=b.id_kelas_kuliah');
		$this->db->join($_ENV['DB_GREAT'].'mata_kuliah c','c.id_matkul=b.id_matkul');
		$this->db->join($_ENV['DB_GREAT'].'dosen d','d.nidn=a.input_by');
		// $this->db->join($_ENV['DB_GREAT'].'ajar_dosen e','e.id_kelas_kuliah=a.id_kelas_kuliah and a.input_by=e.nidn');
		$this->db->join($_ENV['DB_REF'].'prodi p','p.kode_prodi=b.kode_prodi');
        $this->db->join($_ENV['DB_REF'].'fakultas f','f.kode_fak=p.kode_fak');
 		// $this->db->group_by('e.id_kelas_kuliahs');
        $this->db->group_by(['d.nm_sdm', 'a.id_kelas_kuliah']);
